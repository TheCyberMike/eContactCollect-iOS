//
//  WizSendEmailDefine16ViewController.swift
//  eContact Collect
//
//  Created by Dev on 8/31/19.
//

import UIKit
import Eureka

class WizSendEmailDefine16ViewController: FormViewController {
    // member variables
    var mFirstTime:Bool = true
    
    // member constants and other static content
    private let mCTAG:String = "VCW4-16"
...
//debugPrint("\(self.mCTAG).viewDidAppear STARTED")
        ...
        let potentialVia:EmailVia? = EmailHandler.shared.getKnownProvider(forEmailAddr: self.mRootVC!.mWorking_EmailVia!.sendingEmailAddress)
        if potentialVia != nil && self.mFirstTime {
            let vc = AdminPrefsEditEmailAccountViewController()
            vc.mEditVia = potentialVia
